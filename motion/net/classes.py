...
class stNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.slayer1 = GraphConv(4,64)
        self.slayer2 = GraphConv(64,64)
        self.tlayer3 = GraphConv(64,64)
        self.slayer4 = GraphConv(64,64)
        self.slayer5 = GraphConv(64,64)
        self.tlayer6 = GraphConv(64,64)
        self.slayer7 = GraphConv(64,64)
        self.slayer8 = GraphConv(64,64)
        self.tlayer9 = GraphConv(64,64)
        self.layer10 = GraphConv(64,3)
...
#         self.dropout = nn.Dropout(p=0.5)
    ...
    def forward(self, sg,tg, features):
        x = F.leaky_relu(self.slayer1(sg, features))
        x = F.leaky_relu(self.slayer2(sg, x))
        x = F.leaky_relu(self.tlayer3(tg, x))
        x = F.leaky_relu(self.slayer4(sg, x))
        x = F.leaky_relu(self.slayer5(sg, x))
        x = F.leaky_relu(self.tlayer6(tg, x))
        x = F.leaky_relu(self.slayer7(sg, x))
        x = F.leaky_relu(self.slayer8(sg, x))
        x = F.leaky_relu(self.tlayer9(tg, x))
        x = self.layer10(sg, x)
